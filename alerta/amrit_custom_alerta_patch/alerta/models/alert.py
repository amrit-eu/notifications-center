...
class Alert:
    ...
    # get events
    @staticmethod
    def get_events(query: Query = None) -> List[str]:
        return db.get_alert_events(query)
    
    # get resources
    @staticmethod
    def get_resources(query: Query = None) -> List[str]:
        return db.get_alert_resources(query)
    ...
    def from_expired(self, text: str = '', timeout: int = None):
        return self.from_action(action='close', text='alert has expired', timeout=timeout)
