...
class Backend(Database):
    ...
    def get_alert_events(self, query=None, topn=1000):
        query = query or Query()
        select = """
            SELECT environment, event, count(1) FROM alerts
            WHERE {where}
            GROUP BY environment, event
        """.format(where=query.where)
        return [{'environment': t.environment, 'event': t.event, 'count': t.count} for t in self._fetchall(select, query.vars, limit=topn)]

    #ALERTS RESOURCES
    def get_alert_resources(self, query=None, topn=1000):
        query = query or Query()
        select = """
            SELECT environment, resource, count(1) FROM alerts
            WHERE {where}
            GROUP BY environment, resource
        """.format(where=query.where)
        return [{'environment': t.environment, 'resource': t.resource, 'count': t.count} for t in self._fetchall(select, query.vars, limit=topn)]
    ...
    def get_expired(self, expired_threshold, info_threshold):
        ...
        select = """
            SELECT *
              FROM alerts
             WHERE status NOT IN ('expired', 'closed') AND COALESCE(timeout, {timeout})!=0
