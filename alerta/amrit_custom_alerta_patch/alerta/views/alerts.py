...
# get alert events
@api.route('/alerts/events', methods=['OPTIONS', 'GET'])
@cross_origin()
@permission(Scope.read_alerts)
@timer(gets_timer)
@jsonp
...
# get alert resources
@api.route('/alerts/resources', methods=['OPTIONS', 'GET'])
@cross_origin()
@permission(Scope.read_alerts)
@timer(gets_timer)
@jsonp
def get_resources():
    query = qb.alerts.from_params(request.args, customers=g.customers)
    resources = Alert.get_resources(query)
