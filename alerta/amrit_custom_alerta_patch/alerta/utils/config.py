...
class Config:
    ...
    def get_user_config():
        ...
        # ========== ADDED ENV VAR FOR AMRIT NEEDS =============
        config['ALLOWED_JWT_ROLES'] = get_config('ALLOWED_JWT_ROLES', default=[], type=list, config=config)
        config['VERIFY_JWT_JWKS_ENDPOINT'] = get_config('VERIFY_JWT_JWKS_ENDPOINT', default=None, type=str, config=config)
        config['USER_ROLES'] = get_config('USER_ROLES', default=[], type=list, config=config)
        config['USER_DEFAULT_SCOPES'] = get_config('USER_DEFAULT_SCOPES', default=[], type=list, config=config)
        config['DELETE_SCOPES'] = get_config('DELETE_SCOPES', default=[], type=list, config=config)
        # ======================================================
